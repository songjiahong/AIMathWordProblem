...
from create_dataset.read_from_pkl import data_from_pkl
from create_dataset.read_from_json import data_from_json

drive_root = 'C:/TempFiles/math/'
...
def transformer_data():
    ...
    data = data_from_pkl()
    input_exps = list(data['Question'].values)
    target_exps = list(data['Equation'].apply(lambda x: convert_equation(x)).values)
    ...
    # Creating training and validation tf datasets
